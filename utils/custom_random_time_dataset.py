...
class CustomRandomTimeDataset(data.Dataset):
    def __init__(self, dataset_root: str, n_range=None, transform=None, phase='train'):
        self.dataset_root = dataset_root
        self.n_range = n_range
        self.n = 0 if self.n_range is None else self.n_range[0]
        ...
        self.target_indexes = random.sample(range(self.__len__()), k=self.__len__())
    ...
    def __getitem__(self, index):
        ...
        times = []
        for i in range(3):
            if len(self.target_indexes) == 0:
                if self.n_range is not None:
                    self.n = random.randint(self.n_range[0], self.n_range[1])
                self.target_indexes = random.sample(range(self.__len__()), k=self.__len__())
            target_index = self.target_indexes.pop()
            images.append(self.fp_set['image'][target_index + self.n * i])
            labels.append(self.fp_set['label'][target_index + self.n * i])
            times.append(target_index + self.n * i)
