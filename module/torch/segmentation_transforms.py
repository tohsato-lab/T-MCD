...
class ConvertBrightness(object):
    def __init__(self):
        x = np.arange(256)
        self.convert_map = 0.45 * x

    def convert(self, img):
        return cv2.LUT(np.asarray(img), self.convert_map).astype(np.float32)

    def __call__(self, kwargs: dict) -> dict:
        return {
            key: np.asarray(kwargs[key]) if 'label' in key else self.convert(kwargs[key]) for key in kwargs.keys()
        }
